## Next
* Standardize `Module.fromX` conversions on their `Module.toX` counterparts (e.g. `Nat32.fromNat8` → `Nat8.toNat32`, `Float.fromInt` → `Int.toFloat`, `Text.fromChar` → `Char.toText`, `Blob.fromArray` → `Array.toBlob`, `Int8.fromInt` → `Int.toInt8`). Every `fromX` with a `toX` counterpart across `Nat`, `NatN`, `Int`, `IntN`, `Float`, `Float32`, `Text`, `Char`, `Blob`, `Array`, `VarArray`, and `Iter` now emits a regular `@deprecated` warning naming the replacement (instead of the caffeine-only `M0235` lint). Restores `List.get`, `Map.{swap,replace,take}`, `Set.{deleteAll,insertAll}`, and the `Result.Result` type alias — these had no equivalent replacement. `validate:docs` now treats `@deprecated` (`M0154`) usages as errors so docs examples can never reference deprecated APIs (#501).
